...
import com.tech.world.activities.calling.AgoraConfig
import com.tech.world.activities.calling.CallingActivity
import com.tech.world.activities.calling.event.CallingStateEvent
import com.tech.world.activities.calling.event.CallingViewState
import com.tech.world.activities.calling.model.*
import com.tech.world.common.extensions.abandonAudioFocusCompat
import com.tech.world.common.extensions.requestAudioFocusCompat
import com.tech.world.model.constants.FireCallDirection
import com.tech.world.model.realms.FireCall
import com.tech.world.utils.*
import com.tech.world.utils.network.CallsManager
...
class CallingService : Service(), ProximitySensor.Delegate, AudioManager.OnAudioFocusChangeListener,
    DuringCallEventHandler {
                    ...
                    IntentUtils.NOTIFICATION_ACTION_START_INCOMING -> {
                        ...
                        RealmHelper.getInstance().getFireCall(callId)?.let { fireCall ->
                            ...
                            val incomingCallNotification =
                                NotificationHelper(this@CallingService).createIncomingCallNotification(
                                    fireCall,
                                    getNotificationId()
                                )
    ...
    private fun stop() {
        ...
        LocalBroadcastManager.getInstance(this)
            .sendBroadcast(Intent(IntentUtils.ACTION_FINISH_CALLING_ACTIVITY))
        stopForeground(true)
    ...
    private fun requestAudioFocus(streamType: Int) {
        audioFocusRequest =
            audioManager?.requestAudioFocusCompat(this, streamType, AudioManager.AUDIOFOCUS_GAIN)
    }
    ...
    private fun startForeground() {
        ...
        fireCall?.let { fireCall ->
            val activeCallNotification =
                NotificationHelper(this).createActiveCallNotification(fireCall, getNotificationId())
    ...
    private fun onCallEstablished() {
        ...
        fireCall?.let { call ->
            if (call.direction == FireCallDirection.INCOMING) {
                ...
                val activeCallNotification =
                    notificationHelper.createActiveCallNotification(call, getNotificationId())
    ...
    fun startCall(fireCall: FireCall, isIncoming: Boolean) {
        ...
        if (joinChannelResult == 0) {
            if (!isIncoming) {
                ...
                if (fireCall.isGroupCall) {
                    callManager.saveOutgoingGroupCallOnFirebase(fireCall, fireCall.user.uid)
                        .subscribe({
                    ...
                    callManager.listenForEndingCall(fireCall.callId, fireCall.user.uid, isIncoming)
                        .subscribe({
                            if (it.exists()) {
                                endCall(CallEndedReason.REMOTE_HUNG_UP)
                            }
                    ...
                    if (isVideoCall) {
                        videoUids.keys.forEach { uid ->
                            ...
                            videoUids[uid]?.let { isMuted ->
                                ...
                                postViewEvent(
                                    CallingViewState.MuteOrUnmuteRemoteViewForUid(
                                        uid,
                                        isMuted
                                    )
                                )
