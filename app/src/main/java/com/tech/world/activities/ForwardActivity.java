...
import com.tech.world.adapters.ForwardAdapter;
...
import com.tech.world.views.DevlomiSnackbar;
...
public class ForwardActivity extends BaseActivity {
    ...
    private void handleIncomingAudio() {
        Uri uri = (Uri) getIntent().getParcelableExtra(Intent.EXTRA_STREAM);
        String filePath = BuildVerUtil.isApi29OrAbove() ? uri.toString() : RealPathUtil.getRealPath(this, uri);
        if (filePath == null) {
    ...
    private void handleVideoShare() {
        ...
        String filePath = BuildVerUtil.isApi29OrAbove() ? videoUri.toString() : RealPathUtil.getRealPath(this, videoUri);
        ...
        if (adapter.getSelectedForwardedUsers().size() > 1) {
            ...
            for (User user : adapter.getSelectedForwardedUsers()) {
                Message message = new MessageCreator.Builder(user, MessageType.SENT_VIDEO).context(this).path(filePath).fromCamera(false).build();
                if (message != null) {
                    ServiceHelper.startNetworkRequest(this, message.getMessageId(), message.getChatId());
                }
            }
    ...
    private void handleImageShare(List<User> selectedUsers) {
        ...
        if (imageUris != null) {
            ...
            String filePath = BuildVerUtil.isApi29OrAbove() ? imageUri.toString() : RealPathUtil.getRealPath(this, imageUri);
            if (filePath == null) {
                showFileNotFoundToast();
                return;
            }
    ...
    private List<String> getRealPathList(ArrayList<Uri> imageUris) {
        if (BuildVerUtil.isApi29OrAbove()) {
            return UriStringMapper.mapListOfUriToString(imageUris);
        } else {
