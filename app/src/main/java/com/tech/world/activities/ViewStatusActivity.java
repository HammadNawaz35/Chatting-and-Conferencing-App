...
import com.tech.world.adapters.StatusSeenByAdapter;
import com.tech.world.adapters.StatusSeenByCallback;
import com.tech.world.model.ExpandableContact;
import com.tech.world.model.constants.MessageType;
import com.tech.world.model.constants.StatusType;
import com.tech.world.model.constants.TypingStat;
...
import com.tech.world.model.realms.StatusSeenBy;
import com.tech.world.model.realms.TextStatus;
import com.tech.world.model.realms.User;
import com.tech.world.placespicker.Place;
import com.tech.world.placespicker.PlacesPickerActivity;
import com.tech.world.utils.BitmapUtils;
import com.tech.world.utils.BuildVerUtil;
import com.tech.world.utils.ContactUtils;
import com.tech.world.utils.DirManager;
import com.tech.world.utils.DpUtil;
import com.tech.world.utils.FileFilter;
import com.tech.world.utils.FileUtils;
import com.tech.world.utils.FireConstants;
import com.tech.world.utils.IntentUtils;
import com.tech.world.utils.MessageCreator;
import com.tech.world.utils.MyApp;
import com.tech.world.utils.RealPathUtil;
import com.tech.world.utils.RealmHelper;
import com.tech.world.utils.RecorderSettings;
import com.tech.world.utils.ServiceHelper;
import com.tech.world.utils.SharedPreferencesManager;
import com.tech.world.utils.StatusHelper;
import com.tech.world.utils.TimeHelper;
import com.tech.world.utils.UriStringMapper;
import com.tech.world.utils.Util;
import com.tech.world.utils.keyboard.KeyboardHeightObserver;
import com.tech.world.utils.keyboard.KeyboardHeightProvider;
import com.tech.world.utils.keyboard.KeyboardHelper;
import com.tech.world.utils.keyboard.KeyboardUtils;
import com.tech.world.utils.mediastore.MediaStoreFileInfo;
import com.tech.world.utils.network.FireManager;
import com.tech.world.utils.network.StatusManager;
import com.tech.world.views.AnimButton;
import com.tech.world.views.AttachmentView;
import com.tech.world.views.sticker.FireStickerProvider;
...
import static com.tech.world.activities.main.messaging.ChatActivity.CAMERA_REQUEST;
import static com.tech.world.activities.main.messaging.ChatActivity.MAX_FILE_SIZE;
import static com.tech.world.activities.main.messaging.ChatActivity.PICK_CONTACT_REQUEST;
import static com.tech.world.activities.main.messaging.ChatActivity.PICK_GALLERY_REQUEST;
import static com.tech.world.activities.main.messaging.ChatActivity.PICK_LOCATION_REQUEST;
import static com.tech.world.activities.main.messaging.ChatActivity.PICK_MUSIC_REQUEST;
import static com.tech.world.activities.main.messaging.ChatActivity.PICK_NUMBERS_FOR_CONTACT_REQUEST;
import static com.tech.world.activities.main.messaging.ChatActivity.RECORD_START_AUDIO_LENGTH;
import static com.tech.world.utils.FontUtil.isFontExists;
...
public class ViewStatusActivity extends BaseActivity implements StoriesProgressView.StoriesListener, PullBackLayout.Callback, StatusSeenByCallback, KeyboardHeightObserver {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //this is used when clicking on replied status.
        statusId = getIntent().getStringExtra(IntentUtils.EXTRA_STATUS_ID);
        if (statusId != null) {
            statuses = RealmHelper.getInstance().getStatuses(statusId);
            if (statuses.isEmpty()) {
                Toast.makeText(this, R.string.status_not_exists, Toast.LENGTH_SHORT).show();
                finish();
                return;
            }
        } else {
        ...
        cameraBtn.setOnClickListener(view -> startCamera());
    ...
    private void getTypingLayoutHeight() {
        typingLayout.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {

            @Override
            public void onGlobalLayout() {
                typingLayout.getViewTreeObserver().removeOnGlobalLayoutListener(this);
                typingLayoutHeight = typingLayout.getMeasuredHeight();
            }
        });
    }
    ...
    private void initMediaPlayer() {
        ...
        onPreparedListener = new MediaPlayer.OnPreparedListener() {
            ...
            public void onPrepared(MediaPlayer mp) {
                if (isCurrentIndexValid(counter)) {
    ...
    private void initViews() {
        ...
        root_constraint = findViewById(R.id.root_constraint);
    ...
    private void loadStatus(final Status status) {
        ...
        root_constraint.setBackgroundColor(Color.BLACK);
    ...
    private void loadTextStatus(TextStatus textStatus) {
        try {
            ...
            root_constraint.setBackgroundColor(Color.parseColor(color));
    ...
    public void onPull(float v) {
        ...
        if (isFinishing
                ...
                || isReplyExpanded())
            return;
        int height = root_constraint.getHeight();
        float newAlpha = 1 - v;
        float trans = height * v;
        if (v >= 0.2) {
            root_constraint.animate().translationY(height).setDuration(200).start();
            root_constraint.animate().alpha(0).setDuration(200).start();
            isFinishing = true;
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
            ...
            root_constraint.animate().alpha(newAlpha).setDuration(0).start();
            root_constraint.animate().translationY(trans).setDuration(0).start();
        }
    }
    ...
    public void onPullCancel() {
        if (currentVelocity <= 0.2) {
            ...
            root_constraint.animate().translationY(0).setDuration(200).start();
            root_constraint.animate().alpha(1).setDuration(200).start();
        }
        resume();
    }
...
//        finish();
    ...
    public void onKeyboardHeightChanged(int height, int orientation) {
        //height > 0, keyboard is visible, otherwise it's hidden
        if (height > 0) {
            float margin = DpUtil.toPixel(10, this);
            bottomSheetReply.animate().translationY(-height - typingLayoutHeight - margin).start();
        } else {
            bottomSheetReply.animate().translationY(0).start();
        }
    }
}
