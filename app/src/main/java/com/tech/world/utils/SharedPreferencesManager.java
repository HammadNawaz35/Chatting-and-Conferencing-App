...
import com.tech.world.model.constants.NetworkType;
...
public class SharedPreferencesManager {
    ...
    public static boolean canDownload(int mediaType, int availableNetworkType) {
        //if it's a voice message or a sticker download it automatically
        if (mediaType == MessageType.RECEIVED_VOICE_MESSAGE
                || mediaType == MessageType.RECEIVED_STICKER) return true;
    ...
    public static void setAppVersion(String appVersion) {
        mSharedPref.edit().putString("appVersion", appVersion).apply();
    }

    public static String getAppVersion() {
        return mSharedPref.getString("appVersion", "-1");
    }
    ...
    public static void setE2ESaved(boolean bool) {
        mSharedPref.edit().putBoolean("e2e_saved", bool).apply();
    }
    ...
    public static void setDeviceIdSaved(boolean bool) {
        mSharedPref.edit().putBoolean("device_id_saved", bool).apply();
    }

    public static void setHasEnteredUsername(boolean bool) {
        mSharedPref.edit().putBoolean("has_entered_username", bool).apply();
    }
    ...
    public static void setFinishedRestoreBackup(boolean b) {
        mSharedPref.edit().putBoolean("finished_restore_backup", b).apply();
    }
    ...
    public static void saveSetupInfo(String username, String localPhotoPath, String backupUri,String dbFileUri) {
        mSharedPref.edit().putString("username", username).apply();
        if (localPhotoPath != null) {
            mSharedPref.edit().putString("localPhotoPath", localPhotoPath).apply();
        }
        if (backupUri != null) {
            mSharedPref.edit().putString("backupUri", backupUri).apply();
        }

        if (dbFileUri != null) {
            mSharedPref.edit().putString("dbFileUri", dbFileUri).apply();
        }
