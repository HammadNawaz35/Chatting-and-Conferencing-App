package com.tech.world.model.realms;
...
public class Message extends RealmObject implements Parcelable, Comparable {
    ...
    //used to get users Cards when using E2EE.
    private RealmList<String> broadcastUids = new RealmList<>();
    @Ignore
    Status status;
    ...
    public void setBroadcastUids(List<String> broadcastUids) {
        RealmList realmList = new RealmList<String>();
        realmList.addAll(broadcastUids);
    ...
    public Map<String, Object> toMap() {
        ...
        result.put(DBConstants.ENCRYPTION_TYPE, encryptionType);
        ...
        if (contact != null) {
            ...
            result.put(DBConstants.CONTACT, contact.getJsonString());
        }

        //if there is a location in this message add it to the map
        if (location != null) {
            boolean isEncrypted = !encryptionType.equalsIgnoreCase(EncryptionType.NONE);

            result.put(DBConstants.LOCATION, location.toMap(isEncrypted));
        }

        if (partialText != null) {
            result.put(DBConstants.PARTIAL_TEXT, partialText);
        }
    ...
    public Message(String messageId, String fromId, String fromPhone,
                   String toId, int type, String content, String timestamp,
                   String chatId, int messageStat, String localPath, int downloadUploadStat,
                   String metadata, boolean voiceMessageSeen, String mediaDuration,
                   String thumb, boolean isForwarded, String videoThumb, String fileSize,
                   RealmContact contact, RealmLocation location, boolean isGroup,
                   boolean isBroadcast, boolean isSeen, Status status, QuotedMessage quotedMessage,
    ...
    public Message getClonedMessage() {
        ...
        message.setBroadcastUids(broadcastUids);
        return message;
    }
    ...
    public boolean isMediaType() {
        return
                        ...
                        type == MessageType.SENT_STICKER ||
                        type == MessageType.RECEIVED_STICKER;
    ...
    public boolean isStickerType() {
        return type == MessageType.SENT_STICKER || type == MessageType.RECEIVED_STICKER;
    }
}
